...
1. Заранее установите Ollama и загрузите нужные модели.
2. Переименуйте `config.ini-sample` в `config.ini` и внесите нужные параметры.
3. Запустите собранный jar из домашней/рабочей директории с нужными настройками, например:
...
Если меняли `server.port`, откройте тот же хост с вашим портом.

## Инструкции по установке

Инструкции по установке приложения и сервиса находятся в директории `installation/`:
После установки и запуска откройте в браузере: `http://localhost:8080` (или ваш порт, если меняли `server.port`).
